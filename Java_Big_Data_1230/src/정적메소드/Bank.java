...
public class Bank {
    private static int count = 0; // 해당 클래스가 객체로 몇번 생성되는지 확인하는 변수
    ...
    public static int getCount(){  // 정적 필드값을 읽기 위한 정적 메소드
        return count;
    }
    // 예금을 위한 메소드 작성
    public void setDeposit(final int dep) { // 입금된 고객의 금액을 더이상 변경 불가 상태로 만들어 한전하게 보호 함.
        this.account += dep;
    ...
    public void setWithdraw(final int with) {
        ...
        System.out.println(with + "을 출금하였습니다.");
    }
